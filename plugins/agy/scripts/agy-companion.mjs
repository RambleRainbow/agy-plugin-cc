...
const MODEL_ALIASES = new Map([["spark", "gpt-5.3-agy-spark"]]);
...
function printUsage() {
  console.log(
    [
      ...
      "  node scripts/agy-companion.mjs task [--background] [--write] [--resume-last|--resume|--fresh] [--model <model|spark>] [--effort <none|minimal|low|medium|high|xhigh>] [prompt]",
...
async function buildSetupReport(cwd, actionsTaken = []) {
  ...
  if (!agyStatus.available) {
    nextSteps.push("Install Agy with `npm install -g @openai/agy`.");
  }
  if (agyStatus.available && !authStatus.loggedIn && authStatus.requiresOpenaiAuth) {
    nextSteps.push("Run `!agy login`.");
    nextSteps.push("If browser login is blocked, retry with `!agy login --device-auth` or `!agy login --with-api-key`.");
  }
...
function ensureAgyAvailable(cwd) {
  ...
  if (!availability.available) {
    throw new Error("Agy CLI is not installed or is missing required runtime support. Install it with `npm install -g @openai/agy`, then rerun `/agy:setup`.");
  }
}
...
async function executeTaskRun(request) {
  ...
  if (request.resumeLast) {
    ...
    if (!latestThread) {
      throw new Error("No previous Agy task thread was found for this repository.");
    }
  ...
  if (interrupt.attempted) {
    appendLogLine(
      ...
      interrupt.interrupted
        ? `Requested Agy turn interrupt for ${turnId} on ${threadId}.`
        : `Agy turn interrupt failed${interrupt.detail ? `: ${interrupt.detail}` : "."}`
    );
  }
