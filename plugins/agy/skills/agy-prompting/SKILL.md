...
Use this skill when `agy:agy-rescue` needs to compose a prompt for Agy.
...
- Use `rescue` when the task is diagnosis, planning, research, or implementation and you need to control the prompt more directly.
- Use `--continue` for follow-up instructions on the same Agy thread. Send only the delta instruction instead of restating the whole prompt unless the direction changed materially.
